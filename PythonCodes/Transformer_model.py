...
class Model(nn.Module):
    def __init__(self, config):
        ...
        self.droupout1 = torch.nn.Dropout(p=0.1)
    ...
    def forward(self, x):
        ...
        out_emb = self.my_score_mutation(out)
    ...
    #     # out = torch.mean(out, 1)
    #     index = train_idx
    #     out = self.my_score_mutation(out,index,expain_num,expain_num+1)
    #     out = self.fc1(out)
    #     out = self.relu1(out)
    #     out = self.droupout1(out)
    #     out = self.fc2(out)
...
class My_score_mutation(nn.Module):
    ...
    def forward(self, x):
        ...
        score_mutation=np.load(self.score_mutation_path)
        ...
        final_tensor=torch.ones(final_tensor.size)
        out = final_tensor.mul(x).to(self.device) 
...
class Scaled_Dot_Product_Attention(nn.Module):
    ...
    def forward(self, Q, K, V, scale=None):
        ...
        Args:
            ...
            scale: square root dim_K
        Return:
            self-attention output Z, attention matrix A 
        '''
        ...
        if add_our_attention:
            ...
            tem_tensor=tem_tensor.unsqueeze(0).repeat(attention.shape[0], 1, 1).to(config.device)
